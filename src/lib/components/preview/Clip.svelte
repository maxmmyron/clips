<script lang="ts">
  ...
  $: if (clip.type === "video" && $current.video.includes(clip.uuid)) {
    ...
    if (!$player.isPaused && buffer.paused) {
      buffer.currentTime =
        $timeline.runtime - clip.metadata.offset - clip.metadata.start;
    ...
    buffer &&
    clip.type === "video" &&
    !$current.video.includes(clip.uuid)
  ) {
    buffer = buffer as HTMLVideoElement;
    buffer.pause();
  }
  ...
  $: if (clip.type === "audio" && $current.audio.includes(clip.uuid)) {
    gainNode.gain.value = clip.metadata.volume;
    if (!$player.isPaused && !hasAudioNodeStarted) {
      audioNode = $audioContext.createBufferSource();
      audioNode.buffer = (
        $media.resolved.find((media) => media.uuid === clip.mediaUUID) as
          | App.Audio
          | App.Video
      ).metadata.audio;
  ...
  const handleMove = (e: MouseEvent) => {
    ...
    if (isMove) {
      ...
      const linkedClip = <App.Clip>(
        $timeline.clips[clip.type === "audio" ? "video" : "audio"][
          clip.metadata.trackIdx
        ].get(clip.linkUUID)
      );
    ...
    if (isResize) {
      ...
      if (resizeDirection == "left")
        clip.metadata.start = Math.min(
          clip.metadata.runtime - 1 / 60,
          Math.max(0, (e.clientX - initialX) / $secondWidth)
        );
      // otherwise, clamp total runtime between the current start offset and the total duration of the clip
      else
        clip.metadata.runtime = Math.min(
          initialRuntime - (initialResizePosition - e.clientX) / $secondWidth,
          clip.metadata.duration - clip.metadata.start
        );
  ...
  const handleClick = (e: MouseEvent) => {
    if (e.shiftKey) selected = [...selected, clip.uuid];
      ...
      let track =
        clip.type === "audio"
          ? $timeline.clips.audio[clip.metadata.trackIdx]
          : $timeline.clips.video[clip.metadata.trackIdx];
      clip.metadata.z =
        Math.max(...[...track.values()].map((clip) => clip.metadata.z)) + 1;
...
<button
  ...
  style="width: {(clip.metadata.runtime - clip.metadata.start) *
    $secondWidth}px; transform: translateX({(clip.metadata.offset +
    clip.metadata.start) *
...
>
  {#if clip.type === "audio"}
    {#key $timeline.zoomScale || clip.metadata.runtime}
      <MediaAudioPreview
        mediaUUID={clip.mediaUUID}
        start={clip.metadata.start}
        runtime={clip.metadata.runtime}
      />
      <input
        ...
        max="2"
        step="0.01"
        bind:value={clip.metadata.volume}
        on:mousedown={() => (isControllingVolume = true)}
      />
      <p class="text-white absolute top-0 right-0">
  ...
  {/if}
  <div
    class="absolute bottom-0 left-0 w-full py-0.5 px-1 bg-neutral-800/75 backdrop-blur-lg"
  >
...
{#if clip.type === "video"}
  <video
    muted
    class="pointer-events-none opacity-[0.000000001]"
    src={clip.src}
    bind:this={buffer}
  />
